...
export function middleware(request) {
  ...
  const isLanguageRoot = supportedLanguages.some(
    lang => pathname === `/${lang}/`
  );
  ...
  if (
    pathParts.length === 2 &&
    pathParts[0] === 'algorithms' &&
    validAlgorithms.includes(pathParts[1].toLowerCase())
  ) {
    const url = request.nextUrl.clone();
  ...
  if (
    pathParts.length === 3 &&
    supportedLanguages.includes(pathParts[0]) &&
    pathParts[1] === 'algorithms' &&
    validAlgorithms.includes(pathParts[2].toLowerCase())
  ) {
    const url = request.nextUrl.clone();
