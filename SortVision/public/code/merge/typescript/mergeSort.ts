...
function mergeSort(arr: number[], left: number, right: number): number[] {
  ...
  if (!arr || left < 0 || right >= arr.length || left > right) {
    ...
    if (left > right) {
      ...
      console.error(
        `Invalid input for mergeSort: arr.length=${arr.length}, left=${left}, right=${right}`
      );
...
function main() {
  ...
  const sortedExampleArray = mergeSort(
    exampleArray,
    0,
  ...
  function runTestCase(testName: string, input: number[]) {
    ...
    try {
      ...
      const passed =
        result.length === expectedSorted.length &&
      ...
      if (passed) {
        ...
        console.log(
          `  Status: FAILED - Expected: ${JSON.stringify(expectedSorted)}`
        );
      }
    } catch (e: any) {
      // Type 'any' for e in catch block to access message safely
