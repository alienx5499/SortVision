...
async function testUrlsInBatches(urls, batchSize = MAX_CONCURRENT_REQUESTS) {
  ...
  console.log(
    `${colors.cyan}Testing ${urls.length} URLs in ${batches.length} batches of ${batchSize}...${colors.reset}\n`
  );
  ...
  for (let i = 0; i < batches.length; i++) {
    ...
    process.stdout.write(
      `${colors.blue}[Batch ${batchNumber}/${batches.length}]${colors.reset} Testing ${batch.length} URLs... `
    );
    ...
    console.log(
      `${colors.green}[OK] ${successCount}/${batch.length} successful${colors.reset}`
    );
...
function generateReport(results) {
  ...
  console.log(
    `${colors.green}[OK] Successful (200 OK): ${successful.length}${colors.reset}`
  );
  ...
  if (redirects.length > 0) {
    ...
    console.log(
      'These URLs are redirecting and should be updated in the sitemap:\n'
    );
...
function saveResults(results, outputPath) {
  ...
  console.log(
    `${colors.green}[OK] Results saved to: ${outputPath}${colors.reset}`
  );
}
...
async function main() {
  ...
  try {
    ...
    if (urls.length === 0) {
      console.error(
        `${colors.red}[ERROR] No URLs found in sitemap${colors.reset}`
      );
      process.exit(1);
    }

    console.log(
      `${colors.green}[OK] Found ${urls.length} URLs in sitemap${colors.reset}\n`
    );
    ...
    if (!report.success) {
      console.log(
        `${colors.red}[FAILED] Tests failed: ${report.errorCount} errors, ${report.redirectCount} redirects${colors.reset}`
      );
      process.exit(1);
    } else {
      console.log(
        `${colors.green}[SUCCESS] All tests passed successfully!${colors.reset}`
      );
      process.exit(0);
    }
  } catch (error) {
    console.error(
      `${colors.red}[FATAL] Fatal error: ${error.message}${colors.reset}`
    );
    process.exit(1);
  }
}
