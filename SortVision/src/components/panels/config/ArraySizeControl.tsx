...
import type { ArraySizeControlProps } from './configPanelContracts';
import {
  ARRAY_SIZE_MAX,
  ARRAY_SIZE_MIN,
  ARRAY_SIZE_QUICK_DELTA,
  ARRAY_SIZE_SLIDER_STEP,
  normalizedArraySizeRatio,
} from './visualizerControlConstants';

const ArraySizeControl = ({
  arraySize,
  setArraySize,
  isSorting,
      ...
      <div className="relative bg-slate-900 p-4 rounded border border-slate-800 transition-all duration-500 hover:border-slate-700 hover:shadow-lg hover:shadow-slate-900/50 group/array overflow-hidden h-full">
        ...
        <div className="absolute inset-0 overflow-hidden opacity-10">
          <div className="absolute top-0 left-0 w-full h-full">
            ...
            ></div>
            <div
              className="absolute h-1.5 w-1.5 rounded-full bg-blue-500/50 top-[70%] left-[30%] animate-pulse"
        ...
        <div className="relative mt-6 mb-8">
          ...
          <div className="relative">
            ...
            <div
              className="absolute h-full bg-blue-400/20 rounded-full overflow-hidden origin-left"
              style={{
                transform: `scaleX(${normalizedArraySizeRatio(arraySize)})`,
          ...
          <div className="absolute -bottom-6 left-0 right-0 flex justify-between text-[10px] text-slate-500">
            <span className="group-hover/array:text-blue-300 text-blue-300 transition-colors duration-300">
              {ARRAY_SIZE_MIN}
            </span>
            <span className="group-hover/array:text-blue-300 text-blue-300 transition-colors duration-300">
              100
        ...
        <div className="flex justify-between items-center mt-2 text-xs text-slate-400">
          ...
          <div className="flex space-x-1">
            ...
            >
              ...
              <div className="absolute inset-0 flex items-center justify-center text-blue-400 font-mono text-[10px]">
                -10
              </div>
            ...
            <button
              type="button"
              onClick={() => {
                if (!isSorting && arraySize < ARRAY_SIZE_MAX) {
                  const newSize = Math.min(
                    ARRAY_SIZE_MAX,
                    arraySize + ARRAY_SIZE_QUICK_DELTA
                  );
