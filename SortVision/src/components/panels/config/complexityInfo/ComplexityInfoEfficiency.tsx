...
import {
  ...
  resolveEfficiencyBand,
} from './complexityConfig';
import type { ComplexitySectionProps } from './complexitySectionTypes';

const ComplexityInfoEfficiency = ({
  complexity,
  t,
}: ComplexitySectionProps) => {
  const band = resolveEfficiencyBand(complexity.efficiency);
  const colorClass =
    EFFICIENCY_TEXT_COLOR_CLASSES[complexity.color] ||
    EFFICIENCY_TEXT_COLOR_CLASSES.default;
  const iconConfig = EFFICIENCY_ICON_CONFIG[band];
