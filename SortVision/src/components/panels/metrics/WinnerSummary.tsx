...
import { getAlgorithmCategory } from './algorithmCategories';
import type { SortedMetricListEntry } from './metricsPanelContracts';

export type WinnerSummaryProps = {
  sortedMetrics: SortedMetricListEntry[];
};

const WinnerSummary = ({ sortedMetrics }: WinnerSummaryProps) => {
  if (sortedMetrics.length === 0) return null;

  const winner = sortedMetrics[0]!;
  const slowest = sortedMetrics[sortedMetrics.length - 1]!;

  const winnerCategory = getAlgorithmCategory(winner.algo);
  const slowestCategory = getAlgorithmCategory(slowest.algo);

  const winnerTimeMs = Number.parseFloat(String(winner.metrics.time ?? '0'));
  const slowestTimeMs = Number.parseFloat(String(slowest.metrics.time ?? '0'));
  const speedMultiplier =
    winnerTimeMs > 0 ? Math.round(slowestTimeMs / winnerTimeMs) : 0;
  const timeDeltaMs = slowestTimeMs - winnerTimeMs;
    ...
    <div className="mt-4 pt-4 border-t border-slate-700">
      <div className="bg-slate-800 p-3 rounded border border-yellow-500 relative overflow-hidden group hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300">
        ...
        <div className="text-sm text-slate-400 font-mono flex items-center justify-between relative z-10">
          ...
          <Badge
            variant="outline"
            className="bg-yellow-500/20 text-yellow-500 border-yellow-500/30 group-hover:bg-yellow-500/30 transition-colors duration-300"
          >
            WINNER
          </Badge>
        </div>
