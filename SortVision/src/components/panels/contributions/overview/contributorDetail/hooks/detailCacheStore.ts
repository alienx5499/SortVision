...
export type DetailCacheSnapshot = {
  timestamp: number;
  profileData: unknown;
...
const inMemoryDetailCache = new Map<string, DetailCacheSnapshot>();

function getStorageCacheMap(): Record<string, DetailCacheSnapshot> {
  if (typeof window === 'undefined') return {};
  try {
    return JSON.parse(
      sessionStorage.getItem(DETAIL_CACHE_STORAGE_KEY) || '{}'
    ) as Record<string, DetailCacheSnapshot>;
  } catch {
    return {};
  }
}

function setStorageCacheMap(
  cacheMap: Record<string, DetailCacheSnapshot>
): void {
  if (typeof window === 'undefined') return;
  try {
...
export function readDetailCache(cacheKey: string): DetailCacheSnapshot | null {
  const now = Date.now();
