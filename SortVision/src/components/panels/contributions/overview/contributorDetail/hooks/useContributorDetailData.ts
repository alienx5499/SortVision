...
export function useContributorDetailData({
  ...
  isOpen: boolean;
  authenticatedFetch: AuthenticatedFetch | undefined | null;
  getCachedContributorStats?: (
    login: string
  ...
  const loadMoreCommits = useCallback(async () => {
    ...
    try {
      let statsPayload: CachedContributorLineTotals | null | undefined = null;
      if (getCachedContributorStats) {
      ...
      const commitsData = (await response.json()) as unknown[];
      ...
      setCommits(prevCommits => {
        const seen = new Set(prevCommits.map(c => (c as { sha?: string }).sha));
        const merged = [...prevCommits];
        for (const commit of enhancedIncoming) {
          const sha = (commit as { sha?: string }).sha;
          if (sha && !seen.has(sha)) {
            merged.push(commit);
          }
        }
        ...
        writeDetailCache(cacheKey, {
          timestamp: Date.now(),
          profileData: existingMemory?.profileData ?? null,
          pullRequests: existingMemory?.pullRequests ?? [],
          issues: existingMemory?.issues ?? [],
          commits: merged,
