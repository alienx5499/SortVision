...
const RepositoryHealth = () => {
  ...
  const fetchHealthData = useCallback(async () => {
    try {
      ...
      const recentlyUpdatedIssues = actualOpenIssues.filter(
        issue =>
          ...
          >
            <RefreshCw
              className={`h-3 w-3 text-slate-500 hover:text-blue-400 transition-colors ${
                healthData.loading ? 'animate-spin' : ''
              }`}
            />
          </button>
        </div>
            ...
            <div className="p-3 bg-slate-800/50 rounded-lg border border-slate-700">
              ...
              <div className="space-y-2">
                <div className="font-mono text-sm text-white">
                  {healthData.releases.latest.name ||
                    healthData.releases.latest.tag_name}
                </div>
                <div className="font-mono text-xs text-slate-400">
                  {new Date(
                    healthData.releases.latest.published_at
                  ).toLocaleDateString()}
                </div>
                <a
