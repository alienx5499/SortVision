  ...
  try {
    ...
    assignedIssues.forEach(issue => {
      if (!issue.pull_request) {
        // Exclude PRs, same as leaderboard
...
export const exportToExcel = async onProgress => {
  try {
    ...
    for (let i = 0; i < activeParticipants.length; i++) {
      ...
      const contributorData = await fetchContributorDetails(
      ...
      const exportData = {
        ...
        'Beginner Score':
          contributorData.beginnerIssues * POINTS_CONFIG.Beginner,
        'Intermediate Score':
          contributorData.intermediateIssues * POINTS_CONFIG.Intermediate,
        'Advanced Score':
          contributorData.advancedIssues * POINTS_CONFIG.Advanced,
