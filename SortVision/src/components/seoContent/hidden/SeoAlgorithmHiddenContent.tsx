...
export const SeoAlgorithmHiddenContent = ({
  ...
  const details = (ALGORITHM_DETAILS as Record<string, any>)[
    normalizedAlgorithm
  ];
  const algorithmName =
  ...
  return (
    <div className="sr-only" aria-hidden="true">
      ...
      <div data-geo="prompt-hooks" style={{ display: 'none' }}>
        {(promptHooks as any[]).map((hook: string, index: number) => (
          <p key={index}>{hook}</p>
        ))}
      </div>
      ...
      {details ? (
        <>
          ...
          <ul>
            {(details.useCases as string[]).map((useCase: string) => (
          ...
          <ul>
            {(details.realWorldUse as string[]).map((item: string) => (
              <li key={item}>{item}</li>
            ))}
          </ul>
          ...
          <ul>
            {(details.interviewTips as string[]).map((tip: string) => (
              <li key={tip}>{tip}</li>
            ))}
          </ul>
          ...
          <ul>
            {(details.commonMistakes as string[]).map((mistake: string) => (
