const GEMINI_ENDPOINT =
  process.env.NEXT_PUBLIC_GEMINI_ENDPOINT || 'http://localhost:3001/api/gemini';
...
function generateContextualResponse(query, context) {
  ...
  if (containsKeyword(query, KEYWORDS.complexity)) {
    return algoData
      ...
      : generateBasicAlgorithmInfo(displayName, context);
  }
            ...
            <div class="animate-fade-in space-y-1 max-w-full">
                ...
                ${
                  array && array.length > 0
                    ? `<p class="m-0 text-xs text-emerald-300">Current array: [${array
                        .slice(0, 8)
                        .join(', ')}${array.length > 8 ? '...' : ''}]</p>`
                    : ''
                }
            </div>`
      : generateBasicAlgorithmInfo(displayName, context);
  ...
  return algoData
    ? `
        <div class="animate-fade-in space-y-1 max-w-full">
            ...
            <p class="m-0 text-sm">${algoData.description}</p>
            <div class="flex gap-3 text-xs">
                <span class="text-emerald-300">⏱️ ${
                  algoData.timeComplexity
                }</span>
                <span class="text-blue-300">💾 ${
                  algoData.spaceComplexity
                }</span>
            </div>
            ${
              array && array.length > 0
                ? `<p class="m-0 text-xs text-slate-300">Array size: ${array.length} elements</p>`
                : ''
            }
