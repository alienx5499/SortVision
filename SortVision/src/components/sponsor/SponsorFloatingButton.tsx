...
import type { SponsorFloatingButtonProps } from './sponsorContracts';

export default function SponsorFloatingButton({
  onClick,
}: SponsorFloatingButtonProps) {
    ...
    <div className="fixed top-4 right-6 z-50 group max-sm:top-[max(1rem,env(safe-area-inset-top))]">
      <div className="relative">
        ...
        <Button
          ...
          className="relative size-16 rounded-full shadow-2xl transition-all duration-500 bg-gradient-to-br from-rose-500 via-rose-400 to-amber-500 hover:from-rose-400 hover:via-rose-500 hover:to-amber-400 border-2 border-rose-200/50 hover:border-amber-200/80 overflow-hidden group-hover:scale-110 group-hover:rotate-3 active:scale-95"
        ...
        >
          ...
          <div className="absolute inset-0 rounded-full overflow-hidden">
            ...
            />
            <div
              className="absolute bottom-3 left-3 size-0.5 bg-rose-200 rounded-full animate-ping"
          ...
          <Heart
            className={`size-8 relative z-10 transition-all duration-500 fill-rose-950/20 text-rose-950 drop-shadow-lg ${
              isHovered
        ...
        <div className="absolute inset-0 pointer-events-none">
          {Array.from({ length: 6 }, (_, i) => (
            <div
              key={i}
              className="absolute size-1 bg-rose-300 rounded-full animate-ping opacity-60"
              style={{
