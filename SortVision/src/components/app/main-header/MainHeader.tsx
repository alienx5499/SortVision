...
const MainHeader = memo(({ getLocalizedUrl }: MainHeaderProps) => (
  <header className="flex flex-col items-center mb-4 sm:mb-6 animate-fade-down animate-once animate-duration-[800ms] animate-delay-100">
    <div className="flex items-center gap-2 sm:gap-3">
      <Terminal
        className="size-6 sm:size-8 text-emerald-400 animate-pulse animate-infinite animate-duration-[3000ms]"
        aria-hidden="true"
      />
      ...
      <Code
        className="size-4 sm:size-6 text-slate-400 animate-spin animate-once animate-duration-[1500ms] animate-delay-300"
        aria-hidden="true"
      />
    </div>
