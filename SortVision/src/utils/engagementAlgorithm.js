...
export class EngagementTracker {
  ...
  getMetrics() {
    const timeSinceStart =
      Date.now() - (this.interactionHistory[0]?.time || Date.now());
    const interactionsPerMinute =
      this.interactionCount / (timeSinceStart / 60000) || 0;
  ...
  calculateWeightedScore(timeSpent) {
    ...
    const velocityMultiplier = Math.min(
      1.5,
      1 + metrics.interactionsPerMinute / 20
    );
 ...
 */
export function shouldShowPopupImproved(
  tracker,
  timeSpent,
  popupType = 'github'
) {
  ...
  if (
    timeSpent >= config.fastTrack.time &&
    weightedScore >= config.fastTrack.score
  ) {
    return {
      shouldShow: true,
      reason: 'fastTrack',
