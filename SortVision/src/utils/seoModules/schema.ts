 ...
 */
export const getAlgorithmSchema = (algorithmName: string, path: string) => {
  const algorithm = (algorithms as Record<string, any>)[algorithmName];
...
export const getAllAlgorithmUrls = () => {
  return Object.keys(algorithms).map(key => ({
    url: `/algorithms/${key}`,
    title: (algorithms as Record<string, any>)[key].name,
    description: (algorithms as Record<string, any>)[key].seo_description,
 ...
 */
export const getAlgorithmHowToSchema = (algorithmName: string) => {
  const algorithm = (algorithms as Record<string, any>)[algorithmName];
  if (!algorithm) return null;
...
export const getAlgorithmCatalogSchema = () => {
  ...
  return {
    ...
    itemListElement: algorithmKeys.map((key, index) => ({
      '@type': 'ListItem',
      position: index + 1,
      name: (algorithms as Record<string, any>)[key].name,
      url: `${baseUrl}/algorithms/config/${key}`,
      description: `Interactive ${(algorithms as Record<string, any>)[key].name} visualization with real-time animations and performance metrics`,
    })),
  };
};
 ...
 */
export const getComparisonContext = (algorithmNames: string[]) => {
  ...
  const algorithmNamesList = algorithmNames
    .map(name => (algorithms as Record<string, any>)[name]?.name)
    .filter(Boolean)
    .join(' vs ');
